<template>
  <v-card >
    ...
    >
      ...
      <v-app-bar-title>شتاب دهنده امین </v-app-bar-title>
      <v-spacer></v-spacer>
      ...
      <template v-slot:extension>
        <v-tabs background-color="#5B7BB6" show-arrows class="">
          <v-tab v-for="(item, index) in navItem" :key="index"  :to="item.link"
            >{{ item.name }}
          </v-tab>
        </v-tabs>
      </template>
